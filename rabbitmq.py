...
class TornadoRabbitMQConsumer(object):
    """This consumer will handle unexpected interactions
    ...
    def __init__(self, host='localhost', port=5672, user='guest', password='guest', vhost=''):
        ...
        self._url = 'amqp://%s:%s@%s:%d/%s' % (user, password, host, port, vhost)
        log.trace('amqurl=%s' % self._url)
    ...
    def start(self, exchange, exchange_type, queue, routing_key):
...
class AsyncRabbitMQPublisher(object):
    ...
    def publish_message(self):
        ...
        # switch interval if there is no message queued
        if len(self._messages) == 0:
            if self.PUBLISH_INTERVAL < 10:
                self.PUBLISH_INTERVAL = 10
            return
        else:
            self.PUBLISH_INTERVAL = self._publish_interval

        message = self._messages.pop(0)

        properties = pika.BasicProperties(app_id='rabbitmq-async-publisher',
    ...
    def run(self, exchange, exchange_type, queue, routing_key, publish_interval=1):
        ...
        self._publish_interval = self.PUBLISH_INTERVAL
...
class RabbitMQConsumer(TornadoRabbitMQConsumer):
    pass

class RabbitMQPublisher(AsyncRabbitMQPublisher):
    pass
